...
/** Unused in the current iteration, if it is unused in the next few iterations, remove*/
public class ListenedMultiFluidTank implements IFluidHandler {
    ...
    public ListenedMultiFluidTank(IFluidHolder holder) {
        getTanks = () -> {
            return holder.getFluidSize();
        };
        getMaxTanks = () -> {
            return holder.getFluidCapacity();
        };
        getFluidInTank = tank -> {
            return holder.getFluidStack(tank);
        };
        fill = fluid -> {
            return holder.insertFluid(fluid);
        };
        drain = index -> {
            return holder.removeFluid(index);
        };
    }
